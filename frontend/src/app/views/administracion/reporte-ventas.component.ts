...
export class ReporteVentasComponent implements OnInit, AfterViewInit {
  ...
  generarQueryString(): string {
    ...
    const fecha_desde = this.reporteForm.value.fecha_desde
      .toISOString()
      .split('T');
    const fecha_hasta = this.reporteForm.value.fecha_hasta
      .toISOString()
      .split('T');
    ...
    for (const key in this.reporteForm.value) {
      ...
      if (value) {
        // Solo añade parámetros con valores
        if (Array.isArray(value)) {
          // Manejo de arrays (para filtros múltiples)
  ...
  downloadCSV() {
    ...
    this.renderedData.forEach((e: any) => {
      // Usa forEach para iterar
      e.ventas_detalle.forEach((detalle: any) => {
        // Itera sobre ventas_detalle
        this.csvList.push({
          // Añade un nuevo objeto a csvList por cada detalle
          fecha: e.fecha,
          empresa: e.empresa,
          vendedor: e.vendedor,
          cliente: e.cliente,
