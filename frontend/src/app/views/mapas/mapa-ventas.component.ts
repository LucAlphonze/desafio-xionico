...
export class MapaVentasComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    const initialState = { lng: -64.1888, lat: -31.4201, zoom: 10 };
    ...
    this.http
      .get<any[]>(
        `${this.reporteVentasUrl}?fecha_desde=2025-01-01&fecha_hasta=2025-02-12&empresa=1&page_size=50`
      )
      .subscribe((data: any) => {
        // Obtén los datos de la API
        data.results.forEach((venta: any) => {
          // Usa forEach para iterar
          venta.ventas_detalle.forEach((detalle: any) => {
            // Itera sobre ventas_detalle
            const marker = new Marker({ color: '#FF0000' })
              .setLngLat([venta.long, venta.lat])
              .setPopup(
                new Popup().setHTML(
